...
2. Ask `software_developer` to invoke `$develop-story` with the approved design
   and implement the bounded product change.
3. Ask `test_engineer` to invoke `$test-story` with the resulting diff and
   produce acceptance evidence.
4. Ask `code_reviewer` to invoke `$review-story` only after implementation and
   tests are available.
